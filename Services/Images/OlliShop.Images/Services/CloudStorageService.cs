...
namespace OlliShop.Images.Services
{
            ...
            try
            {
                ...
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    using (var storageClient = StorageClient.Create(_googleCredential))
                    {
                        ...
                        // Dosyayı public olarak ayarlıyoruz
                        var acl = await storageClient.GetObjectAsync(_options.GoogleCloudStorageBucketName, fileNameToSave);
                        storageClient.UpdateObject(acl, new UpdateObjectOptions { PredefinedAcl = PredefinedObjectAcl.PublicRead });

                        // Public URL'yi döndürüyoruz
                        string publicUrl = $"https://storage.googleapis.com/{_options.GoogleCloudStorageBucketName}/{fileNameToSave}";
                        return publicUrl;
                    }
                }
            }
